---
title: "Connect Claude Code to your Salesforce org with MCP"
description: "The six-phase setup: Node + SF CLI + Claude Max + the Salesforce DX MCP server + a single .mcp.json file. 15 minutes from zero to your first prompt."
date: 2026-04-23
author: Amit
...
- **An IDE** — this post uses IntelliJ with Illuminated Cloud 2. The MCP config works the same way with VS Code + Salesforce Extension Pack, Cursor, or any editor.

None of these are optional. If you're missing any, the later phases will fail in confusing ways.

## Phase 2 — Create your SFDX project

In IntelliJ: **File → New → Project → Illuminated Cloud SFDX**. Name the project and add an org connection. Your browser opens to authenticate against the Salesforce org you want to use. Once you authenticate, IntelliJ downloads and indexes your org's metadata.

If you're on VS Code instead, run:
...
Either way, the goal is a Salesforce DX project folder with `sfdx-project.json` at the root and a valid org alias you can reference later.
